...
class AIImageDetector:
    ...
    def analyze_artifacts(self, image_path):
        ...
        try:
            ...
            noise_thresh_1 = 2.0 * s_thresh  # Reduced from 2.5
            noise_thresh_2 = 3.5 * s_thresh  # Reduced from 4.0
            if noise_level < noise_thresh_1:
                ai_score += 25 * s_score_mult # Reduced from 40
                explanations.append(f"Extremely low sensor noise ({noise_level:.2f}) - typical of AI")
            elif noise_level < noise_thresh_2:
                ai_score += 15 * s_score_mult # Reduced from 25
            ...
            if fft_ratio < fft_low or fft_ratio > fft_high:
                ai_score += 20 * s_score_mult # Reduced from 25
            ...
            if edge_density > edge_thresh:
                ai_score += 10 * s_score_mult # Reduced from 15
                explanations.append(f"High edge density ({edge_density:.3f}) - perfect edges")
            ...
            if texture_std < texture_thresh:
                ai_score += 5 * s_score_mult # Reduced from 10
                explanations.append(f"Very smooth texture ({texture_std:.1f})")
                ...
                if custom_result:
                    ...
                    if self.sensitivity == 'high':
                        try:
                            ...
                            if artifact_result and artifact_result.get('metrics', {}).get('ai_score', 0) >= 65: # Raised threshold
